...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match := re.match(r"add_chat\((.+?)\)", query.data):
        ...
        if is_kuki := sql.set_kuki(chat.id):
            ...
            update.effective_message.edit_text(
                f"Hey Darling Hydra Chatbot enable by {mention_html(user.id, user.first_name)}.",
                parse_mode=ParseMode.HTML,
            )
...
def chatbot(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(
            f"http://api.brainshop.ai/get?bid=171693&key=qZsmlB6XQGnKnotE&uid=[user]&msg=[message]"
        )
